...
It contains two Terraform roots:
...
A single platform repository contains:
...
3. GitHub Actions runs:
   - `terraform init` with the GCS backend
   - `terraform plan` against the appropriate environment project
4. The plan is reviewed and the pull request is merged
5. GitHub Actions on `main`:
   - requests an OIDC token
   - uses Workload Identity Federation to assume the environment service account
   - runs `terraform apply` against the same state and project
